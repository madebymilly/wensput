function Products({ products }) {
  ...
  return (
    <>
      ...
      {products.map(({ id, name = '', allPaLeeftijd = '', allPaMinAantalSpelers = '', allPaSpeelduur = '', allPaTaal = '', allPaThema = [], productCategories = [], productTags= [], slug = '' }) => (
        <div key={id} style={{ padding: '20px' }}>
          <h3>{name}</h3>
          <h4>Slug: {slug}</h4>
          <h5>Audio-file: '/audio/games/{slug}.mp3'</h5>
          {allPaLeeftijd.nodes.length > 0 ? <p>Leeftijd: {allPaLeeftijd.nodes[0].name} / {allPaLeeftijd.nodes[0].slug}</p> : ''}
